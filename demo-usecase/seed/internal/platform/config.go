...
// OneOfDefault returns an OPTIONAL variable constrained to a fixed set. Unset yields
// def; set-but-invalid fails validation exactly as OneOf does, so a typo is reported
// rather than silently falling back to the default.
func (e *Env) OneOfDefault(key, def string, allowed ...string) string {
	v := strings.TrimSpace(os.Getenv(key))
	if v == "" {
		return def
	}
